...
export async function POST(request: Request) {
  ...
  // Delete folders (this will cascade to files inside via RLS/triggers)
  if (folderIds && folderIds.length > 0) {
    // Get all files in folders recursively for storage cleanup
    ...
    async function getFilesInFolder(folderId: string) {
      ...
      if (files) {
        for (const file of files) {
          allFileIds.push(file.storage_path)
        }
      }
    ...
    if (allFileIds.length > 0) {
      await supabase.storage.from("files").remove(allFileIds)
    }
