...
export function MoveDialog({ open, onOpenChange, onMove, excludeFolderIds = [], isMoving }: MoveDialogProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="flex items-center gap-1 text-sm text-slate-600 flex-wrap">
          <button onClick={() => navigateUp(0)} className="flex items-center gap-1 hover:text-slate-900">
          ...
          {currentPath.map((folder, index) => (
            <div key={folder.id} className="flex items-center gap-1">
              ...
              <button onClick={() => navigateUp(index + 1)} className="hover:text-slate-900">
                {folder.name}
              </button>
            </div>
          ))}
        ...
        <ScrollArea className="h-64 border rounded-md">
          {loading ? (
            ...
            <div className="p-4 text-center text-slate-500">No folders here</div>
          ) : (
            <div className="p-2">
              {folders.map((folder) => (
                <div
                  ...
                  className={cn(
                    ...
                    selectedFolder === folder.id && "bg-indigo-50 ring-1 ring-indigo-200",
                  )}
        ...
        <p className="text-xs text-slate-500">Double-click to navigate into a folder, or select and click Move.</p>
